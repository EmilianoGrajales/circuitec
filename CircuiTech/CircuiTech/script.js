var i = 0;
var txt = "Hi! I'm Renz Bautista Sayaman";
var speed = 100;
var pauseDuration = 2000;

function typeWriter() {
  if (i < txt.length) {
    document.getElementById("headLabel").innerHTML += txt.charAt(i);
    i++;
  } else {
...
function deleteText() {
  if (i > 0) {
    document.getElementById("headLabel").innerHTML = txt.substring(0, i - 1);
    i--;
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  window.addEventListener("scroll", function () {
    ...
    if (scrollPosition > scrollThreshold) {
      ...
      coverContainer.classList.add("scrolled");
    } else {
      menuContainer.classList.remove("scrolled");
      coverContainer.classList.remove("scrolled");
    }
  });
});
